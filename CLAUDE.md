...
**NAP consistente (desde 2026-08-26):** el Google Business Profile (Place ID arriba) ya se renombró a **Clínica Hispana Nueva Salud Lawndale**, igual que el sitio. Su URL en GBP lleva UTM (`?utm_source=google&utm_medium=organic&utm_campaign=gmb`); el perfil declara idiomas español/inglés y estacionamiento gratuito en el lugar. Las URLs de Maps/reseña en `CONTACT_INFO` van ancladas al Place ID.

**PENDIENTE del cliente (no bloquea el build, sí el go-live de tracking):**
- Credenciales de tracking que falten: Google Ads conversion labels (`NEXT_PUBLIC_GADS_CONVERSION_LABEL_LANDING_CALL` sigue vacía — TODO en `landing-conquesting.ts`). **CallRail SÍ está activo en producción** (`NEXT_PUBLIC_CALLRAIL_SWAP_URL` existe en Vercel aunque no en `.env.example`; verificado 2026-09-08): `swap.js` rota números de pool por sesión, así que el número visible difiere del real (832) 530-4188 que llevan HTML, schema y GBP como teléfono adicional. El principal del GBP, (832) 743-2344, también es de CallRail, a propósito.
- Confirmar **cuáles de los 29 servicios** ofrece realmente. **Civil Surgeon USCIS: confirmado por el cliente el 2026-09-08.** Autor del blog: "Equipo Médico de la clínica" (decisión del cliente; no hay médico nombrado).
...
- Reviews/rating: la entidad `MedicalClinic` estable (sin rating) sale del layout `(default)`; la calificación y las 5 reseñas en vivo van SOLO en la home vía `JsonLdClinicRating` (mismo `@id`, se fusionan). Así el `reviewCount` no difiere entre páginas regeneradas por ISR. Fallback `GOOGLE_REVIEWS_DATA` hoy 4.7/107. Nunca reintroducir reseñas inventadas hardcodeadas.
...
- **Fonts:** Sora (headings, `--font-sora`) and Inter (body, `--font-inter`), loaded via `next/font/google` in `[locale]/layout.tsx`. Don't add other Google fonts.
...
- **Blog markdown:** rendered by `src/components/blog/post-markdown.tsx` (react-markdown + remark-gfm, so tables work). It strips the leading `# Title` from the body (the page already emits the frontmatter title as the h1) and prefixes internal links with `/en` in English. Don't re-add a markdown `# Title` line to existing posts. Frontmatter `dateModified` feeds the visible "Actualizado" date, `BlogPosting.dateModified`, `MedicalWebPage.lastReviewed` and the sitemap `lastmod`: bump it on real content edits.
...
- `src/lib/constants.ts` is ~100 KB. Prefer `Read` with `offset`/`limit` over reading the whole file when editing. Every `Service` has a required `dateModified` (YYYY-MM-DD) that feeds the sitemap `lastmod`: bump it when title/copy/FAQs change, not on refactors.
- **Contenido duplicado en la red:** los posts y buena parte del copy de servicios son idénticos a chlacaridad.com, clinicahispanaairline.com, hispananuevasalud.com y otros dominios hermanos (auditoría 2026-09-08). Es la causa más probable de que Google no indexe. Todo texto nuevo o reescrito debe ser único de Lawndale; no copiar de La Caridad.
- **Auditoría SEO 2026-09-08** (63/100) y plan de acción en `~/Desktop/seo-audit-lawndale-2026-09-08/`; playbook reutilizable en `~/Desktop/playbook-seo-geo-clinicas.md`.
...
- **Google Ads healthcare policy:** be cautious adding prescription-drug-adjacent terms (e.g. "disfunción eréctil"/"erectile dysfunction") to a service page `title`/`description`/`keywords` — they can reduce Quality Score of running Ads campaigns. The full `longDescription` body is lower-risk for the same terms. There is no `/services/urologia` on this site; the only exposure was "testosterona" on `/services/salud-hombre`, removed from its title/description on 2026-09-09 (kept in the body).
